 ...
 */
static inline uint16_t ax25_fcs(uint8_t *buffer, size_t len) {
  uint16_t fcs = 0xFFFF;
  while (len--) {
 ...
 */
static inline size_t ax25_create_addr_field(uint8_t *out, std::string dest_addr,
                                            uint8_t dest_ssid,
                                            std::string src_addr,
                                            uint8_t src_ssid) {
  size_t i;
 ...
 */
static inline uint8_t ax25_get_dest_ssid(const uint8_t *in) {
  uint8_t ret;
  ret = in[AX25_CALLSIGN_MAX_LEN];
  return (ret >> 1) & 0b1111;
}

static inline size_t ax25_prepare_frame(uint8_t *out, const uint8_t *info,
                                        size_t info_len, ax25_frame_type_t type,
 ...
 */
static inline ax25_encode_status_t ax25_nrz_bit_stuffing(
    float *out, size_t *out_len, const uint8_t *buffer, size_t buffer_len,
 ...
 * @return the resulting status of the encoding
 */
static inline ax25_encode_status_t ax25_bit_stuffing(
    uint8_t *out, size_t *out_len, const uint8_t *buffer,
    const size_t buffer_len, size_t preamble_len, size_t postamble_len) {
  uint8_t bit;
...
static inline ax25_decode_status_t ax25_decode(uint8_t *out, size_t *out_len,
  ...
  recv_fcs =
      (((uint16_t) out[received_bytes - 2]) << 8) | out[received_bytes - 1];
